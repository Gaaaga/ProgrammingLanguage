...
class GeometryValue 
  ...
  def intersectLineSegment seg
    ...
    # result of Line intersect with self
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1,x2)
      if y1start < y2start
		  ...
		  else ((aXstart,aYstart,aXend,aYend),(bXstart,bYstart,bXend,bYend)) = [[seg.x1,seg.y1,seg.x2,seg.y2],[x1,y1,x2,y2]]
      end
...
class Let < GeometryExpression
  ...
  def shift (dx,dy)
    Let.new(s,Shift.new(dx,dy,e1),Shift.new(dx,dy,e2))
  end
  def eval_prog env
