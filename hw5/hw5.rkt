...
;; Problem 1
;;a
(define racketlist->mupllist (lambda (rl)
                        (if (null? rl)
                            (aunit)
                            (apair (car rl) (racketlist->mupllist (cdr rl))))))

;;b
(define mupllist->racketlist (lambda (ml)
                               (if (aunit? ml)
                                   null
        ...
        [(call? e)
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               ...
               (eval-under-env (fun-body func) (if (boolean? (fun-nameopt func))
                                                   (cons (cons (fun-formal func) v2) (closure-env v1))
                                                   (cons (cons (fun-formal func) v2)
